...
let sudeste = ['São Paulo', 'Minas Gerais', 'Rio de Janeiro'];
...
brasil.unshift('Amazonas', 'Roraima', 'Para');
...
let primeiroEstadoExcluido = brasil.shift();

console.log(primeiroEstadoExcluido);
...
let nordeste = ['Bahia', 'Alagoas', 'Ceará'];
...
// ?
let maisSeteLetras = brasil.every(function(item, index, array){
...
// ?
let cearaIncluido = brasil.some(function(item, index, array){
    return item === 'Ceará';
});
...
let filter = map.filter(function(item, index, array){
    return item.id % 2 === 0;
})
